...
- Should `POST /session/start` also trigger the state-machine `session_start` transition in Stage 1, or remain a pure session/bundle gate until `POST /handover/request`?
